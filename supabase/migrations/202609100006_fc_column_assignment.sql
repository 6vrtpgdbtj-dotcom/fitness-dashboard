-- PT rows require a unique trainer in column mode. FC sales are branch-level
-- revenue and remain valid when the source has no trainer column.
...
begin
  ...
  if new.record_status='valid' and new.trainer_id is null
    and not (tg_table_name='registrations' and upper(coalesce(to_jsonb(new)->>'product','')) like 'FC %')
...
declare
  ...
  v_name text; v_external text; v_valid boolean; v_trainer_hint text; v_product text;
begin
  ...
  if v_mode='column' then
    ...
    v_product:=nullif(new.record#>>'{values,product}','');
    if new.domain='registration' and v_trainer_hint is null and upper(v_product) like 'FC %' then
      execute format('update public.%I set trainer_id=null,record_status=coalesce($1::public.record_review_status,record_status) where organization_id=$2 and source_connection_id=$3 and source_tab_id=$4 and source_record_key=$5',v_table)
        using new.record->>'record_status',new.organization_id,new.source_connection_id,new.source_tab_id,new.source_record_key;
    else
