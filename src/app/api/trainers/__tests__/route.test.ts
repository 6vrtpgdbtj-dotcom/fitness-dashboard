...
const post = (body: unknown) => POST(new Request("http://localhost/api/trainers", { method: "POST", headers: { origin: "http://localhost" }, body: JSON.stringify(body) }));
beforeEach(() => { vi.stubEnv("NEXT_PUBLIC_APP_URL", "http://localhost"); mocks.role = "admin"; mocks.rpc.mockReset().mockResolvedValue({ data: {}, error: null }); });
