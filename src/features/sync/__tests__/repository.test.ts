...
describe("tab discovery", () => {
  ...
  it("honors explicit PT and FC markers even when both sections have trainer columns", () => {
    const rows = [
      ["PT", "", "", "", "", "", "FC", "", "", "", "", ""],
      ["회원명", "결제 날짜", "매출", "구분", "담당트레이너", "유입경로", "회원명", "결제 날짜", "매출", "구분", "담당트레이너", "유입경로"],
      ["김회원", "2026년 8월 11일", "700,000", "NEW", "정윤수", "필드", "박회원", "8월 12일", "500,000", "신규", "박세준", "상담"],
    ];
    const extracted = extractRepeatedTables(rows, "registration", "26.8");
    expect(extracted).toContainEqual(["김회원", "2026-08-11", "700,000", "NEW", "정윤수", "", "필드", "", "PT 회원권", "결제완료"]);
    expect(extracted).toContainEqual(["박회원", "2026-08-12", "500,000", "신규", "박세준", "", "상담", "", "FC 회원권", "결제완료"]);
  });
  it("uses PT and FC summary labels above a spaced monthly entry table", () => {
    ...
    expect(extractRepeatedTables(rows, "registration", "26.8")).toEqual([
      ["회원명", "결제 날짜", "매출", "RE/NEW", "담당트레이너", "판매트레이너", "유입경로", "결제방법", "상품", "결제상태"],
      ["김회원", "2026-08-10", "900,000", "신규", "정윤수", "", "", "카드", "PT 20회", "결제완료"],
      ["박회원", "2026-08-11", "500,000", "재등록", "", "", "", "현금", "PT 12개월회", "결제완료"],
    ]);
  });
  it("keeps RE/NEW separate from every PT acquisition source in the real monthly layout", () => {
    const rows = [
      ["PT매출", "", "0", "", "", "", "", "FC매출"],
      [], [], [], [],
      ["", "회원명", "세션", "결제 날짜", "매출", "", "", "RE/NEW", "담당트레이너", "유입경로", "결제방법", "", "회원명", "세션", "결제 날짜", "매출", "", "", "RE/NEW", "담당트레이너", "유입경로", "결제방법"],
      ["박세준", "김수미", 20, "8/11", "1,100,000", "", "", "NEW", "박세준", "필드", "카드", "정윤수", "박선주", 10, "8/14", "700,000", "", "", "NEW", "정윤수", "워크인", "카드"],
      ["", "한예정", 50, "8/27", "2,750,000", "", "", "RE", "정윤수", "재등록", "카드"],
      [],
      ["FC", "날짜", "회원명", "개월수", "락커", "운동복", "금액", "결제 방법", "구분", "오티", "특이사항"],
      ["", "8월 2일", "폴 카바나", "1개월", "", "O", "93,500", "카드", "재등록", "X", ""],
    ];
    const extracted = extractRepeatedTables(rows, "registration", "26.8");
    expect(extracted[0]).toContain("유입경로");
    expect(extracted).toContainEqual(["김수미", "2026-08-11", "1,100,000", "NEW", "박세준", "", "필드", "카드", "PT 20회", "결제완료"]);
    expect(extracted).toContainEqual(["박선주", "2026-08-14", "700,000", "NEW", "정윤수", "", "워크인", "카드", "PT 10회", "결제완료"]);
