...
function ReviewRow({ record, members }: { record: ReviewRecord; members: MemberCandidate[] }) {
  ...
  return <tr><th scope="row"><strong>{String(record.name ?? domainLabels[record.domain])}</strong><small>{record.tab_title}</small><small>{record.id.slice(0,8)}</small></th>
    ...
    <td>{record.issues.map(issue => <span className="admin-issue" key={`${issue.field}:${issue.code}`}>{issueLabels[issue.code] ?? "원본 값 확인"}</span>)}{!record.issues.length && "매핑·담당자 확인"}</td>
    <td><button className="quiet-button" aria-expanded={open} onClick={() => setOpen(!open)}>{open ? "닫기" : "검토"}</button>{open && <div className="admin-review-form">
      ...
      <div className="connection-controls"><button className="quiet-button" disabled={state.busy} onClick={() => mutate({ action: "approve", domain: record.domain })}>검토 완료 · 승인</button><button className="danger-button" disabled={state.busy} onClick={() => mutate({ action: "reject", domain: record.domain })}>집계에서 제외</button></div>
